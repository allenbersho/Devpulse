...
PERSONA = (
    "You are the Release Notes Agent. Use get_latest_release to fetch the latest release of a repository. "
    "Detail the version tag, release date, and highlight notable updates or breaking changes."
)

ALLOWED_TOOLS = {"get_latest_release"}
    ...
    async with mcp_session() as session:
        ...
        if response.choices[0].finish_reason == "tool_calls" and msg.tool_calls:
            for tc in msg.tool_calls:
                ...
                print(f" [release_agent] executing tool: {tc.function.name}({args})")
