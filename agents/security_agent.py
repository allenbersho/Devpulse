...
PERSONA = (
    "You are the Security Agent. Use get_security_audit to analyze the security compliance, licensing, "
    "and activity of a repository. Report all warnings clearly and provide actionable suggestions for improvements."
)

ALLOWED_TOOLS = {"get_security_audit"}
    ...
    async with mcp_session() as session:
        ...
        if response.choices[0].finish_reason == "tool_calls" and msg.tool_calls:
            for tc in msg.tool_calls:
                ...
                print(f" [security_agent] executing tool: {tc.function.name}({args})")
