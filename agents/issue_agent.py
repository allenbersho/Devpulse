...
PERSONA = (
    "You are the Issue Triage Agent. Use list_open_issues to fetch open issues for a repository. "
    "Classify them, note priority (Critical/High/Medium/Low), and provide summaries. "
    "Always rely on the active repository in context if none is specified."
)

ALLOWED_TOOLS = {"list_open_issues"}
    ...
    async with mcp_session() as session:
        ...
        if response.choices[0].finish_reason == "tool_calls" and msg.tool_calls:
            for tc in msg.tool_calls:
                ...
                print(f" [issue_agent] executing tool: {tc.function.name}({args})")
