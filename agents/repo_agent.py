...
PERSONA = (
    "You are the Repo Analyst. Use search_repositories, get_repo_details, "
    "or list_contributors to answer with real, current GitHub data. "
    "If a repository context is given, prioritize searching or fetching details for it."
)

ALLOWED_TOOLS = {"search_repositories", "get_repo_details", "list_contributors"}
    ...
    async with mcp_session() as session:
        ...
        if response.choices[0].finish_reason == "tool_calls" and msg.tool_calls:
            for tc in msg.tool_calls:
                ...
                # Auto-inject owner/repo if missing from context
                if ("owner" not in args or "repo" not in args) and tc.function.name != "search_repositories":
                ...
                print(f" [repo_agent] executing tool: {tc.function.name}({args})")
